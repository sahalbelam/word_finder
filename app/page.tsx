...
const Page = () => {
  ...
  const handleRequest = async () => {
    ...
    try {
      ...
      const matches = transcript.filter((item: { text: string }) =>
      ...
      if (error instanceof Error) {
        setError(
          error.message || "An error occurred while fetching the transcript."
        );
      } else {
      ...
      <header className="bg-gray-900 text-white py-6 px-4 shadow-lg">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-gray-400 mt-2">
            Search for specific words in YouTube videos and jump directly to
            those moments
          </p>
        </div>
      </header>
